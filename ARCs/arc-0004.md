...
return a value. The names of arguments **MUST NOT** be included in a
...
An Interfaces designer **SHOULD** try to prevent collisions of method selectors 
between Interfaces that are likely to be implemented together by the same 
application.

> For example, consider the Interface `Calculator` providing addition and subtraction
> of integer methods and the Interface `NumberFormatting` providing formatting
> methods for numbers into strings. If it is likely that these Interfaces will be used together,
> the Interface designer should then ensure that all the method selectors for methods in `Calculator` and
> `NumberFormatting` are distinct.

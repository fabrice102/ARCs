...
on Algorand _Applications_ (aka "smart contracts").
...
Interface **MUST** be unique. Method names **MAY** be non-unique, as long as
...
An Interface designer **SHOULD** try to prevent collisions of method selectors 
...
the **RECOMMENDED** behavior of the Contract is to not
...
a tuple if they are the type of the 15th argument or an argument
further away.)
...
* `afrz` represents an AssetFreezeTx transaction (freeze or unfreeze
  an ASA)
* `appl` represents an ApplicationCallTx transaction (create/invoke an Application)
